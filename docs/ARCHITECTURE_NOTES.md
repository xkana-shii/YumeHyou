...
- Manga details now have active runtime wiring for MangaBaka metadata: `MangaBakaApi` (core-network) + `MediaRepository.getMangaBakaMetadata(...)` (core-domain) resolve MangaBaka series data for AniList manga entries, and `MediaDetailsViewModel`/`MediaDetailsView`/`MediaInformationView` prefer that data in visible manga metadata sections (`/core/network/src/main/java/com/axiel7/anihyou/core/network/api/MangaBakaApi.kt`, `/core/domain/src/main/java/com/axiel7/anihyou/core/domain/repository/MediaRepository.kt`, `/feature/mediadetails/src/main/java/com/axiel7/anihyou/feature/mediadetails/*`).
